...
const products = [
  ...
  { name:"Grated Coconut",price:60,desc:"fresh coconut grated 200gm ₹60",img:"images/grated-coconut.png"},
...
function updateTotal() {
  let total = 0;
  products.forEach((p,i)=> total += p.price * cart[i]);
...
function checkout() {
  ...
  products.forEach((p,i)=>{
    if(cart[i]>0){
      msg += `${p.name} x ${cart[i]} = ₹${p.price*cart[i]}\n`;
    }
  });
